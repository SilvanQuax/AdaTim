...
target_alphas = [[0.342000, 0.684000, '', 5, ntrials],
                 [0.342000, 0.684000, '', 10, ntrials],
                 [0.342000, 0.684000, '', 30, ntrials],
                 [0.342000, 0.684000, '', 50, ntrials],
                 [0.342000, 0.684000, '', 100, ntrials]]

alphas = np.linspace(0.001, 1.3, num=20)

epoc = np.empty((6, ntrials))
loss = np.empty((6, ntrials))
alphaS = np.empty((6, ntrials, 301))
alphaR = np.empty((6, ntrials, 301))
opt_aS = np.empty((6, ntrials))
opt_aR = np.empty((6, ntrials))
epoc[:, :] = np.nan
loss[:, :] = np.nan
alphaS[:, :] = np.nan
alphaR[:, :] = np.nan
...
for talphas in target_alphas:
    ...
    for trial in range(talphas[4]):
        path = 'saved_models_nhidd/LearnAlpha_global_tS%lf_tR%lf_nhidd%i_trial%i' % (tAs, tAr, nhidd, trial + 1) + _des
...
plt.xlim(0.2, 0.6)
plt.ylim(0.4, 0.8)
plt.scatter(opt_aS[0, :], opt_aR[0, :], marker='+', label='n_hidd = 5')
plt.scatter(opt_aS[1, :], opt_aR[1, :], marker='+', label='n_hidd = 10')
plt.scatter(opt_aS[2, :], opt_aR[2, :], marker='+', label='n_hidd = 20')
plt.scatter(opt_aS[3, :], opt_aR[3, :], marker='+', label='n_hidd = 30')
plt.scatter(opt_aS[3, :], opt_aR[3, :], marker='+', label='n_hidd = 50')
plt.scatter(opt_aS[3, :], opt_aR[3, :], marker='+', label='n_hidd = 100')
plt.scatter(0.342000, 0.684000, c='yellow', marker='o', label='target')
plt.xlabel('alpha_s')
plt.ylabel('alpha_r')
plt.legend()
plt.title('Optimal learned time constants')
plt.plot()
# plt.savefig('figures_results/optimal_alphas_nhidd.eps', format='eps', dpi=DPI)
# plt.close()
